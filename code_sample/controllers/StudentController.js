...
exports.list = async (req, res) => {
  ...
  try {
    ...
    const resultsPromise = Student.find()
      .skip(skip)
      .limit(limit)
      .sort({ created: "desc" })
      .populate();
  ...
  try {
    ...
    if (!req.student) {
      return res.status(404).json({
        success: false,
        result: null,
        message: "couldn't found student Profile ",
      });
    }
    let result = {
      _id: req.student._id,
      enabled: req.student.enabled,
...
exports.read = async (req, res) => {
  try {
    // Find document by id
    const tmpResult = await Student.findOne({
      _id: req.params.id,
    });
...
exports.create = async (req, res) => {
  try {
    ...
    const passwordHash = newStudent.generateHash(password);
    ...
    return res.status(200).send({
      ...
      message: "Student document save correctly",
    });
  } catch {
...
exports.update = async (req, res) => {
  try {
    ...
    if (email) {
      ...
      if (existingStudent._id != req.params.id)
        return res
          .status(400)
...
exports.delete = async (req, res) => {
  try {
    ...
    const result = await Student.findOneAndDelete({ _id: req.params.id }).exec();
    // If no results found, return document not found
    if (!result) {
      return res.status(404).json({
        success: false,
...
exports.search = async (req, res) => {
  ...
  try {
    ...
    let result = await Student.find(fields)
      .where("removed", false)
      .sort({ name: "asc" })
      .limit(10);
